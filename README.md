...
Patterns can be applied in myriad contexts. Pattern-driven transduction would likely improve design and runtime efficiency in data engineering workflows that rely heavily on existing regex technology. Parsers and other applications that rely on existing lexical tools (eg, lex/flex) may find that ginr's structured patterns are easier to express and maintain. Distributed real time systems, such as multiplayer online games, that demand concise messaging with relaxed readability requirements can exchange and transduce maximally compact binary byte encoded messages. Nested regular patterns cover a lot of ground, and information architects that employ them liberally will wonder why they ever got on the XML train. Service providers are _de facto_ domain experts and are free to express idiomatic serialization patterns for domain artifacts in persistent stores and in transit, as long as they syntactically differentiate semantic features and satisfy service and consumer needs. Service APIs presenting idiomatic message patterns with embedded serialized artifacts can be consumed by specialized transducers that extend service API patterns to filter, map and reduce features of interest into the consumer domain. This can be done simply, directly and efficiently without involving formal data exchange formats like XML or JSON (see _[Little Use Case](https://github.com/jrte/ribose/wiki#little-use-case-presenting-remote-service-apis)_ for an example). There's a _[Big Use Case](https://github.com/jrte/ribose/wiki#big-use-case-make-it-all-go-away)_ for that.

So microservice and other distributed service providers can publish API messages as nested regular patterns mapping syntactic features in serialized domain artifacts to semantic labels (artifact names) with annotations (range or relational constraints) defined in a service domain dictionary. Consumers of these services can extract syntax from API and extend it to transducer patterns that reduce and assimilate service artifacts into their own domains. In distributed service workflows asynchronous event driven process interactions can be expressed as binary regular patterns mapping events to actions at each processing node. These patterns can be compiled to FSTs, distributed and deployed to controllers at each node to effectively govern local actions and interactions with other processing nodes. Such systems are self-documenting, as service API and deployed controller patterns provide holistic maps of the data and control planes, and controller and API documentation is always up to date with respect to current deployments. Service API and controller patterns provide architects and developers with complete and concise information that is difficult to express and maintain in typical programming idioms, and even more difficult to recover from source code review.
