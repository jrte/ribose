...
The line chart on the left below shows ribose throughput versus regex with transition matrix compression and sum and product traps implemented. The sum and product traps optimize idempotent looping states (sum) and eliminate sequential non-branching states (product). The benchmarking setup was as for the line chart above. The bar chart on the right shows the extent of compression for the ribose linux log transducers before and after compression.
