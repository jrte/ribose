...
The charts below summarize the results of a data extraction benchmarking runoff between Java regex and three ribose variants (see [_Metrics_](https://github.com/jrte/ribose/wiki/Metrics) for more recent benchmarking metrics). These tests were run on an OpenJDK JVM (Temurin-11.0.17+8). The `LinuxKernel` transducer does no lookahead, `LinuxKernelLoose` looks ahead only as far as the distinguishing tag, and `LinuxKernelStrict` looks ahead up to the end of line. Output was muted for the throughput benchmarks shown in the chart on the left and enabled for the GC benchmark chart on the right. All GC benchmark outputs (regex and ribose) were binary equivalent. Initial heap size (2M) and region size (1M) were identical for all GC runs. All three ribose transducers presented similar heap usage profiles, incurring 2 G1 collections and driving the heap to a maximal 4M size (2 young, 1 survivor regions). Regex incurred 6 collections, reclaiming more than 77M of transient object space as the heap grew to 78M (3 young, 1 survivor regions).

![LinuxKernelBench](https://github.com/jrte/ribose/raw/master/etc/markdown/LinuxKernelLog.png)

Size is important in the ribose runtime because transducer transition functions are represented in 2-dimensional arrays (states × inputs) and these are often sparse. Only about a third of the cells in the `LinuxKernelStrict` transition matrix are populated, the rest are dead cells that signal `nul` when hit. Successive transitions may reference cells that are widely distributed in RAM and this can degrade the performance of the L1 data cache, forcing more traffic on the CPU-RAM bus. I vented some steam about this in one of my favorite rants, see _[A Few Words about Time and Space](https://github.com/jrte/ribose/wiki/Stories#a-few-words-about-time-and-space)_ on the wiki.

Ginr automata are state-minimized but the ribose model compiler is able to obtain some lossless compression of ginr FSTs by coalescing equivalent input symbols to eliminate redundancy in the state transition matrix and collapsing non-branching vectors of >1 input bytes or >1 effectors and parameters to eliminate connecting states and transitions. On the input side, this is effected by dropping the intermediate states in non-branching input vectors and matching input against a reference vector in a product trap outside the transition matrix. This also erases the uniqueness of input symbols involved in these vectors, so the granularity of the input equivalence relationship (**Eq**(i, j) ⇔ **∀**s **T**[s, i] = **T**[s, j]) induced by the transition matrix **T** is also reduced and the matrix is compressed in both dimensions.

Additionally, a sum trap optimizes states with predominantly idempotent transitions, looping and testing each input against a bit map of idempotent inputs until an unmapped input is received to force transition to a new state. The sum and product traps operate in very tight loops with small working sets without reference to the transition matrix, which should result in fewer L1 cache misses. However, they may introduce turbulence in the instruction pipeline if they receive frequent shorts bursts of input. The LinuxKernel transducer  consumes a large volume of input in long runs in the sum trap but also involves many short runs in the sum and product traps. This may account for the relatively small improvement in transduction time for this transducer compared to LinuxKernelLoose and LinuxKernelStrict, which involve fewer traps.

![CompressionBenchmarks](https://github.com/jrte/ribose/raw/master/etc/markdown/compressed-throughput.png)

The line chart on the left above shows ribose throughput versus regex with transition matrix compression and sum and product traps implemented. The benchmarking setup was as for the previous line chart. The bar chart on the right shows the number of cells (states × inputs) in the matrices of the ribose linux log transducers before and after compression. For further contrast, the table below shows the dimensions of the ginr automata that the ribose transducers are compiled from. The number of transitions defined for each ginr automaton (G) and compiled ribose transducer (R) are also shown.

|  | States | Inputs | Cells | Transitions(G) | Transitions(R) |
| --: | --: | --: | --: | --: | --: |
| _LinuxKernel_ | 146 | 260 | 37,960 | 3,184 | 824 |
| _LinuxKernelLoose_ | 55 | 260 | 14,300 | 623 | 114 |
| _LinuxKernelStrict_ | 80 | 260 | 20,800 | 1,223 | 166 |

In ginr automata compiled from ribose transducer patterns the input symbols are raw bytes (256) and signals (4 signals are used in these transducers). The ribose model compiler reduces these modulo the input equivalence relation. For some transducers a digit is just a digit, everywhere all the time.
...
Instructions for building ribose models and running transductors are included the javadoc pages in `javadoc/`. The documentation for the `com.characterforming.ribose` package specifies the arguments for the runnable classes `TCompile`, `TDecompile` and `TRun` and presents the ribose runtime service interfaces. The main interfaces are `IRuntime`, `ITransductor` and `ITarget`. The runnable classes can be executed directly or from the shell scripts in `etc/sh`:
