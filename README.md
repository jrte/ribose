...
Ribose suggests that the von Neumann CPU model would benefit from inclusion of finite state transduction logic to coordinate the sequencing of instructions under the direction of FSTs driven by streams of numerically encoded sequential media, and that programming languages should express robust support for `*`-semiring algebra to enable construction of multidimensional regular patterns and compilation to FSTs. Runtime support for a transduction requires little more than a FST stack and a handful of byte[] buffers. The [Burroughs Corporation B5000](https://en.wikipedia.org/wiki/Burroughs_large_systems), produced in 1961, was first to present a stack-oriented instruction set to support nesting of regular patterns of B5000 instructions. Who will be first, in the 21st century, to include analogous instructions to support nesting of FSTs that map regular patterns of bytes to regular patterns of CPU instructions? 

Microservice and other distributed service providers could benefit from a pattern orientation, publishing APIs as idiomatic nested regular patterns mapping syntactic features to semantic features and range constraints, without involving formal data exchange formats like XML or JSON (see _[Little Use Case](https://github.com/jrte/ribose/wiki#little-use-case-presenting-remote-service-apis)_ for an example). APIs presented as nested regular patterns can be directly consumed by developers who, with input patterns in hand, can easily extend them to realize transductions mapping input features of interest into their target domain.

Patterns can also be applied to express asynchronous event driven process interactions in distributed service networks as binary regular patterns mapping events to actions at each processing node. These patterns can be compiled to FSTs, distributed and deployed as controllers at each node to effectively govern local actions and interactions with remote nodes. Such systems would be self-documenting, as controller patterns would provide holistic maps of the control plane, and the documentation would always be up to date with respect to current deployments. The controller patterns provide architects and developers with information that is difficult to express and maintain in typical programming idioms, and even more difficult to recover from source code review. 
