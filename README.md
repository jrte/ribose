...
The charts below summarize the results of a data extraction benchmarking runoff between Java regex and three ribose variants. Initial heap size (2M) and region size (1M) were identical for all runs. The `LinuxKernel` transducer does no lookahead, `LinuxKernelLoose` looks ahead only as far as the distinguishing tag, and `LinuxKernelStrict` looks ahead up to the end of line. All benchmark outputs (regex and ribose) were binary equivalent. All three ribose transducers presented similar heap usage profiles, incurring 2 G1 collections and driving the heap to a maximal 4M size (1M region size, 2 young, 1 survivor). Regex incurred 9 collections, reclaiming more than 450M of transient object space as the heap grew to 80M (1M region size, 7 young, 1 survivor). 
