...
In ginr patterns for ribose transducers the input symbols are raw bytes (256) and signals (4 signals are used in these transducers). In the compiled ribose transducers these are reduced modulo the input equivalence relation induced by the raw transition matrix (eq(i, j) ⇔ ∀s T[s, i] = T[s, j]). For some transducers a digit is just a digit, everywhere all the time.
