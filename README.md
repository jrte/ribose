...
Instructions for building ribose models and running transductors are included the javadoc pages in `javadoc/`. The documentation for the `com.characterforming.ribose` package specifies the arguments for the runnable `Ribose` class and presents the ribose runtime service interfaces. The main interfaces are `IRuntime`, `ITransductor` and `ITarget`. The runnable classes can be executed directly or from the shell scripts in the project root:
